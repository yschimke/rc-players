#!/usr/bin/env bash
# Drive a real swipe through each benchmark player host with XCUITest.
set -euo pipefail
...
derived_data="$repo_root/build/apple-player-benchmark-ui-tests"
result="$repo_root/build/apple-player-benchmark-interaction-ui.xcresult"
...
xcodebuild test -quiet \
  ...
  -only-testing:RemoteComposePlayerUITests/ApplePlayerBenchmarkInteractionUITests
echo "Apple player benchmark interaction UI tests: ok ($result)"
