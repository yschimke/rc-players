...
function manifestIds(inputDir) {
  ...
  if (new Set(ids).size !== ids.length) die("manifest.json contains duplicate ids");
  return ids.sort();
}
...
function validateLane(lanesDir, lane, ids) {
  ...
  const unsupported = filesWith(dir, ".unsupported");
  const known = new Set(ids);
  const extras = [...new Set([...pngs, ...errors, ...unsupported])].filter(
    (id) => !known.has(id),
  );
  if (extras.length) die(`${lane} has results outside the manifest: ${extras.join(", ")}`);
  ...
  for (const id of ids) {
    ...
    if (Number(hasPng) + Number(hasError) !== 1) {
      die(
        `${lane} must have exactly one primary result for ${id}; found ` +
          `${hasPng ? "png" : "no png"} and ${hasError ? "error" : "no error"}`,
      );
    }
    if (unsupported.includes(id) && !hasPng) {
      die(`${lane} reports ${id} unsupported without a rendered PNG`);
    }
  }
  console.log(
    `    ${lane}: ${pngs.length}/${ids.length} png, ${errors.length} error, ` +
      `${unsupported.length} unsupported report`,
  );
}

const [mode, inputDir, lanesDir, ...lanes] = process.argv.slice(2);
if (
  (mode !== "inputs" && mode !== "results") ||
  !inputDir ||
  (mode === "results" && (!lanesDir || lanes.length === 0))
) {
  die(
    "usage: validate-results.mjs inputs <input-dir> | " +
      "results <input-dir> <lanes-dir> <lane>...",
  );
}
const ids = manifestIds(inputDir);
validateInputs(inputDir, ids);
if (mode === "results") {
  if (new Set(lanes).size !== lanes.length) die("lane names must be unique");
  for (const lane of lanes) validateLane(lanesDir, lane, ids);
}
