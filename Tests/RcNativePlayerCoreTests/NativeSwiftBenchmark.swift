// `task_info` is Mach-only, and timing evidence is only meaningful on the Apple hosts the
// benchmark script runs on, so the suite compiles out of the Linux core-only build.
#if canImport(Darwin)
  import Darwin
  import Foundation
