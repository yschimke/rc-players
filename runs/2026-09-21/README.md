...
Measured at `f68cd4751645ab49dc834acf87e5339ad19d2fdc` on `main`.
...
| `native-appkit` | 177 / 241 | 73.4% | 3 / 8 | 244 | 0 |
...
| outcome | golds | what it means |
| --- | ---: | --- |
| both pass | 179 | Settled. Neither lane disagrees. |
| only `typescript` passes | **26** | **The work list**, split below into the player's gaps and this runner's. |
| only `cmp` passes | 17 | The subject is ahead here, or the reference cannot drive the timeline. |
| both fail | 27 | An expectation that survives neither implementation, or a probe neither lane has. |

**21 the player: the reference draws or reports it and this one does not.**
