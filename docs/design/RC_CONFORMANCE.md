...
| `native-appkit` | the Swift core with the macOS sample's AppKit renderer | `raster`, `tree` |
...
asserts raster alone. It is 142 of 249 at the 2026-09-19 `7cd0a46d` measurement, and the tree channel
is where those passes come from. The remaining tree diffs are a work list rather than noise: the
scroll family (the renderer hoists scrolled content, so depths and offsets disagree), `FitBox`
selection, and the Ahem text metrics the golds assert.
