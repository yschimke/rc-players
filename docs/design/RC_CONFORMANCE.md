...
| `native-appkit` | the Swift core with the macOS sample's AppKit renderer | `raster` only |
...
`native-appkit` still answers only `raster`, so its gold count is zero *by construction* and means
nothing. No gold in the corpus asserts raster alone.
