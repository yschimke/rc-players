...
import ee.schimke.composeai.rcembedded.player.RcPlayerChildren
...
import ee.schimke.composeai.rcembedded.player.sortWithPriorities
...
internal fun RcPlayerCollapsible(
  ...
  Layout(content = { RcPlayerChildren(layout) { Modifier } }, modifier = modifier) {
    ...
    fun childWeight(child: Component): Float {
      ...
      return if (vertical) {
        (mods.fastFirstOrNull {
            it is HeightModifierOperation && it.getType() == DimensionModifierOperation.Type.WEIGHT
          } as? HeightModifierOperation)
          ?.getValue() ?: 0f
      } else {
        (mods.fastFirstOrNull {
            it is WidthModifierOperation && it.getType() == DimensionModifierOperation.Type.WEIGHT
    ...
    visitOrder.fastForEach { child ->
      ...
      val explicitVisOp =
        (child as? LayoutComponent)?.selfOrModifier(ComponentVisibilityOperation::class.java)
    ...
    val selfVis = if (visibleCount == 0) Component.Visibility.GONE else Component.Visibility.VISIBLE
    layout.mVisibility = selfVis
    layout.setVisibility(selfVis)
    ...
    if (vertical) {
      ...
      with(horizontalArrangement) { arrange(width, mainSizes, layoutDirection, mainPositions) }
    }
