...
internal class GraphTimeState(
   ...
   *
   * Continuous fields ([RemoteContext.ID_ANIMATION_TIME], [RemoteContext.ID_CONTINUOUS_SEC]) update
   * every frame when [updateContinuous] is true, whereas quantized wall-clock and calendar fields
   * only update when the epoch second advances (and Compose's [MutableFloatState] only emits a
   * snapshot change when the specific field's value actually changes).
   */
  internal fun updateTime(frameMillis: Float, updateContinuous: Boolean = true): Boolean {
    ...
    if (updateContinuous || secondChanged) {
      ...
      continuousSecState.floatValue = lastTimeInSec + Math.floorMod(currentMillis, 1000L) * 1e-3f
      return true
    }
    return false
  }
