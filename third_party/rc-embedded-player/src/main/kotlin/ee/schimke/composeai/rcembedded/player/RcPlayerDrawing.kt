...
import ee.schimke.composeai.rcembedded.player.utils.getPath
import ee.schimke.composeai.rcembedded.player.utils.getTweenPath
  ...
  for (op in operations) {
    ...
    when (op) {
      ...
      is ClipPath -> {
        ...
        val clipOp =
          if (regionOp == ClipPath.PATH_CLIP_DIFFERENCE) ClipOp.Difference else ClipOp.Intersect
      ...
      is DrawText -> {
        ...
        if (full != null && !paintState.textSize.isNaN()) {
          ...
          val end = if (op.mEnd == -1 || op.mEnd > len) len else op.mEnd.coerceIn(start, len)
      ...
      is DrawBitmapScaled -> {
        ...
        val imageId =
          if ((data.imageId and PaintOperation.PTR_DEREFERENCE) != 0) {
            remoteContext.mRemoteComposeState.getInteger(data.imageId and PaintOperation.VALUE_MASK)
          } else {
      ...
      is DrawTextOnCircle -> {
        ...
        if (full != null && !paintState.textSize.isNaN()) {
          ...
          var sweepDegrees = Math.toDegrees((textWidth / finalRadius).toDouble()).toFloat()
          ...
          if (!clockwise) {
            ...
            when (alignment) {
              DrawTextOnCircle.Alignment.CENTER -> finalStartAngle = startAngle - sweepDegrees / 2f
              DrawTextOnCircle.Alignment.END -> finalStartAngle = startAngle - sweepDegrees
              else -> {}
            }
          }
          ...
          run {
            ...
            while (pos < text.length) {
              ...
              yMax =
                maxOf(
                  yMax,
                  (glyph.mBitmapHeight + glyph.mMarginTop + glyph.mMarginBottom).toFloat(),
                )
          ...
          if (width > 0f && pathLength > 0f) {
            ...
            while (pos < text.length) {
              ...
              if (glyphBitmap != null && glyph.mBitmapWidth > 0 && glyph.mBitmapHeight > 0) {
                ...
                val angle =
                  Math.toDegrees(Math.atan2(tangent.y.toDouble(), tangent.x.toDouble())).toFloat()
