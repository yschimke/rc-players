      ...
      if (AnimatedFloatExpression.isMathOperator(v)) {
        ...
        when (opCode) {
          ...
          AnimatedFloatExpression.fromNaN(AnimatedFloatExpression.SQRT) ->
            unary("sqrt") { sqrt(it) }
          AnimatedFloatExpression.fromNaN(AnimatedFloatExpression.ABS) -> unary("abs") { abs(it) }
          AnimatedFloatExpression.fromNaN(AnimatedFloatExpression.SIGN) ->
            unary("sign") { sign(it) }
          AnimatedFloatExpression.fromNaN(AnimatedFloatExpression.EXP) -> unary("exp") { exp(it) }
          AnimatedFloatExpression.fromNaN(AnimatedFloatExpression.FLOOR) ->
            unary("floor") { floor(it) }
          AnimatedFloatExpression.fromNaN(AnimatedFloatExpression.LOG) -> unary("log") { log10(it) }
          AnimatedFloatExpression.fromNaN(AnimatedFloatExpression.LN) -> unary("ln") { ln(it) }
          AnimatedFloatExpression.fromNaN(AnimatedFloatExpression.ROUND) ->
            unary("round") { Math.round(it).toFloat() }
          AnimatedFloatExpression.fromNaN(AnimatedFloatExpression.SIN) -> unary("sin") { sin(it) }
          AnimatedFloatExpression.fromNaN(AnimatedFloatExpression.COS) -> unary("cos") { cos(it) }
          AnimatedFloatExpression.fromNaN(AnimatedFloatExpression.TAN) -> unary("tan") { tan(it) }
          ...
          AnimatedFloatExpression.fromNaN(AnimatedFloatExpression.LOG2) ->
            unary("log2") { log2(it) }
          AnimatedFloatExpression.fromNaN(AnimatedFloatExpression.INV) -> unary("inv") { 1f / it }
          ...
          AnimatedFloatExpression.fromNaN(AnimatedFloatExpression.DUP) -> stack.add(stack.last())
        ...
        val state = varStates[id] ?: throw IllegalArgumentException("State not found for id $id")
