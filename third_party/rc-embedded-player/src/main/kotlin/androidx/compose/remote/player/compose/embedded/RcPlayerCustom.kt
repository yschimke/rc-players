...
internal constructor(
  ...
  public val floatReturns: Set<Int>
    get() =
      rawProperties
        .filter { it.mDataType == Custom.CustomProperty.FLOAT_RETURN }
        .mapTo(HashSet()) { it.mType.toInt() }

  public val textReturns: Set<Int>
    get() =
      rawProperties
        .filter { it.mDataType == Custom.CustomProperty.TEXT_RETURN }
        .mapTo(HashSet()) { it.mType.toInt() }
...
public class CustomPluginRegistry(private val plugins: Map<String, CustomComposablePlugin<*>>) {
  ...
  ) : this(plugins.associateBy { it.name })
 ...
 */
internal val LocalRcCustomPlugins: ProvidableCompositionLocal<CustomPluginRegistry?> =
  compositionLocalOf {
    null
  }
