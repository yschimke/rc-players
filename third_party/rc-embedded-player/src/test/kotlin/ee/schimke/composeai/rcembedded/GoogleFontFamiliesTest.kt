...
class GoogleFontFamiliesTest {
    ...
    fun `two axis sets on one file are two different families`() {
        ...
        val wide =
            resolver.composeFontFamily(GOOGLE_ROBOTO_FLEX, WEIGHT, UPRIGHT, listOf(WDTH to 151f))

        assertNotNull(narrow)
        assertNotNull(wide)
        // The failure this guards: a cache keyed on the family alone hands the second line the
        // first
    ...
    fun `an unvaried request resolves the family's static face`() {
        ...
        // A family, not null. The downloadable-font path this used to defer to needs a font
        // provider,
        // and a Robolectric render without the `FontsContractCompat` shadow has none — which is how
        // the
    ...
    fun `an axis request keeps the requested weight`() {
        // The edge button's label: Medium, plus a `pnum` axis. Instanced at `pnum` alone the file
        // sits
    ...
    private companion object {
        ...
        /**
         * The catalog's own variable face, vendored for the browser lane's font manifest. Reused
         * here rather than downloaded so the test is hermetic — and it is the same file
