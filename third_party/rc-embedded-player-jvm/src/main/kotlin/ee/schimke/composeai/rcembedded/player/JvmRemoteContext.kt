...
internal class JvmRemoteContext(
  ...
  private fun decode(type: Short, width: Int, height: Int, data: ByteArray): ImageBitmap? =
    try {
      when (type) {
        ...
        BitmapData.TYPE_RAW8 -> raster(data, width, height, rowBytes = width, ColorType.ALPHA_8)
        else -> null
      }
    } catch (e: Exception) {
  ...
  /**
   * A raster [ImageBitmap] over straight (unpremultiplied) pixel bytes; null if the buffer is
   * short.
   */
  private fun raster(
    data: ByteArray,
