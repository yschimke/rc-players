...
/**
 * Family names tried, in order, for each core generic id — the first one the host actually has
 * wins.
 *
 * Skia has no notion of a generic family, so unlike Android's `Typeface.SANS_SERIF` and friends
 * there is nothing to ask for directly. The CSS-style names come first because that is what
 * fontconfig resolves on Linux; the concrete names after them are for hosts where those aliases
 * mean nothing (macOS, Windows). Id 0 shares the sans list with id 1 for the same reason Android's
 ...
 *
 * The two prefixes the Android resolver understands are handled the same way it does — strip and
 * look the remainder up as a family name. `google:` is the one that used to end there: on Android
 * it triggers a `FontsContractCompat` download, and with no JVM font provider the name was merely
 * tried locally and fell back to the default face. It is now downloaded instead, through
 * [GoogleFontTypefaceResolver] and the shared machine-local Google Fonts cache, so this seam draws
 * the same file the other lanes resolve that family to. A request the resolver can't serve (no
 ...
 * obvious way to ask for "whatever the default is" — returns exactly that null on Linux, so the
...
private fun resolveSkiaTypeface(spec: TextPaintSpec, context: RemoteContext): Typeface? {
  ...
  fun match(candidates: List<String>): Typeface? = candidates.firstNotNullOfOrNull {
    manager.matchFamilyStyle(it, style)
  }
 ...
 * result out to whole pixels (its out-param is an integer `Rect`); this does not, so bounds here
 * can be fractionally tighter. That is a sub-pixel difference in the anchoring arithmetic, not a
 ...
 * agree; see the shaping note for why `Canvas.drawString` is not used despite matching the
 * framework call one-for-one.
 */
...
/**
 * Resolves [text] to per-glyph (face, id, advance) triples, falling back per character to a face
 * that has the character when [spec]'s own face does not.
 *
...
private fun resolveGlyphsForPath(
  ...
  while (offset < text.length) {
    ...
    if (primary.getStringGlyphs(chars).firstOrNull() == NOTDEF) {
      val fallback = FontMgr.default.matchFamilyStyleCharacter(null, style, emptyArray(), codePoint)
 ...
 *
 * The one of the four with no single call behind it: `Canvas.drawTextOnPath` is Android-only. But
 * it is not hand-rolled either — Skia's own primitive for this is a `TextBlob` of `RSXform`s
 * (rotate + scale + translate, per glyph), which is what the framework builds internally too. So
 * the glyph *placement* is computed here and the drawing is still one Skia call.
 *
 ...
 * three functions. The reason is mechanical, not principled: reconstructing per-glyph *fonts* from
 * a shaped run needs skiko's `RunHandler` callback, and that path segfaults (a use-after-free
 * inside skiko's own ICU run iterator, reproducible with a minimal handler and nothing to do with
 * this code). Placing glyphs from a flattened shaped line instead would silently draw fallback ids
 * against the primary face, which is the worse failure. Tracked as a follow-up; curved text is
 * where this matters least, since per-glyph rotation dominates sub-pixel kerning.
 */
