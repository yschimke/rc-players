...
/**
 * Resolves a document image draw to an [ImageBitmap]. On jvm there is no `Drawable`-typed host
 * image loader (that plumbing stays androidMain), so this is just the embedded decode — the loader
 * fast path the Android seam has for a `BitmapDrawable` has no jvm equivalent yet.
 */
