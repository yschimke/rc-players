...
// 1.0.0-alpha15 (`-sources.jar`), path `player-compose/.../utils/PathUtils.kt`. See the jvm
// module's
// build.gradle.kts header and `third_party/rc-embedded-player/PROVENANCE.md`. The embedded draw
// path
// (`RcPlayerDrawing.kt`) calls these two `RemoteComposeState` extensions to turn core `PathData`
// into
// a Compose `Path`; on Android they come from the `remote-player-compose` AAR, which a
// `kotlin("jvm")`
// module cannot consume — so the neutral source is vendored here for the jvm target. Body
// unchanged;
// its one Android-coupled callee, `FloatsToPath`, is vendored alongside with the conic op swapped
// to
