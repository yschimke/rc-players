...
internal fun RcPlayerText(layout: CoreText, modifier: Modifier) {
  ...
  SynchronousOneLineEllipsisText(
    ...
    Text(
      ...
      maxLines = javaPlayerMaxLines(data.overflow, data.maxLines),
      style =
        if (resolvedFontSize == null) textStyle else textStyle.copy(fontSize = resolvedFontSize),
    )
  }
}
...
private fun SynchronousOneLineEllipsisText(
  ...
  SubcomposeLayout(modifier = modifier) { constraints ->
    ...
    val alignmentLines: Map<AlignmentLine, Int> =
      listOf(FirstBaseline, LastBaseline)
        .mapNotNull { line ->
...
internal fun synchronousEllipsis(
  ...
  while (
    suffixCount < boundaries.lastIndex &&
      measureWidth(text.substring(boundaries[boundaries.lastIndex - suffixCount - 1])) <= halfBudget
  ) {
    suffixCount++
  }
...
private fun graphemeBoundaries(text: String): List<Int> = buildList {
  add(0)
...
/**
 * Map the document's font-family type / name onto a multiplatform [FontFamily] at [weight]/[style].
 *
 ...
 * resolved through the [context] before it is paired with its value. Tags and values are
 * positional, so an axis counts only when both halves are present: pairing a tag with a neighbour's
 * value would draw a real face at silently the wrong instance, which is worse than dropping it.
 */
 ...
 * bytes aren't printable ASCII — a small text id, for instance, unpacks to control characters and
 * is not a tag. Split out (and `internal`) so the unpacking is testable without a `RemoteContext`.
 */
