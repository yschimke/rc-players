...
The controls gallery exercises the segmented picker, stepper, status row, badge row, and disclosure
row. Its SwiftUI reference and diff are generated by the same macOS comparison workflow.

| SwiftUI reference | Remote Compose candidate | Pixel diff |
| --- | --- | --- |
| ![SwiftUI second-tranche controls](swiftui-controls-reference.png) | ![Remote Compose second-tranche controls](after-controls-set.png) | ![SwiftUI versus Remote Compose controls pixel diff](swiftui-controls-vs-remote-diff.png) |

[`controls-comparison.json`](controls-comparison.json) records a **2.76% changed-pixel ratio** at a
16/255 threshold and **2.75/255 mean absolute channel error**. As with the first tranche, the
remaining difference is concentrated in font rasterization and antialiased control edges.
